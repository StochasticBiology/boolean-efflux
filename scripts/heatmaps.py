...
dir_out = os.path.dirname(path_scripts)
...
for motifName in allMotifs:
    ...
    dir_out = rk.create_hm_dir(signal_state, update_rule, motifName, update_method,
                               dir_out)
        ...
        for energy_sim in range(0, number_sims):
            if (energy_sim) % 100000 == 0:
                print('%s: Motif = %s, Update rule: %s, Signal: %s, Energy = %.2f, Probability Threshold = %.4f, Energy Simulation = %s' %
                      (dt.datetime.now().time(), motifName, update_rule, signal_state, energy, prob_threshold, energy_sim))
